...
위와 같은 코드를 작성하면 우리는 `method` 메소드가 호출되었을 때 다음과 같은 순서로 동작하기를 기대한다.
...
2. `increase` 메소드를 호출하면, 아래와 같은 흐름으로 연산을 진행한다.
    ...
    9. `1`을 더한 연산을 마친 후, `a` 변수가 위치한 공간에 값을 다시 write 한다.
...
흔히 싱글톤 패턴을 구현할 때 자주 볼 수 있는 코드 형태이다. (물론 동시성 문제를 고려하여 개선버전이 존재한다.) 마찬가지로 `thread1` 과 `thread2` 메소드가 동시에 수행되었을 때, 해당 코드는 통과할 수 있을까?

정답은 ‘통과하지 못한다’ 이다. 앞서 `Reordering` 절에서도 언급했듯이, `new` 키워드를 이용해 객체를 생성하는 행위는 두 가지로 구분된다.

1. 객체를 생성하기 위한 공간의 할당
2. 객체 변수의 초기화

따라서 1. 객체를 생성하기 위한 공간의 할당이 먼저 선행되고, `thread2` 메소드가 수행되는 경우, instance는 null을 가리키지 않으므로 if 절 안으로 진입하게 된다. 하지만, 객체 변수의 초기화 (`a = 42;`) 는 아직 수행되지 않았으므로(int 변수는 초기화되지 않으면, `0`을 갖는다.) 해당 assert 절은 실패하게 된다.
